...
class XCLIP(CLIP):
    ...
    def cache_text(self, text):
        ...
        with torch.no_grad():
            if self.cache_text_features is None:
                self.cache_text_features = text.mean(dim=1, keepdim=False)
        self.train()
    ...
    def forward(self, image, text):
        ...
        if self.use_cache:
            ...
            text_features = text.mean(dim=1, keepdim=False)
        ...
        # probabilities = F.cosine_similarity(video_features,text_features)
        # print(probabilities)
